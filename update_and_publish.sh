...
# 1) Auto-detect latest source Excel files from Downloads
# 2) Regenerate daily Excel outputs
# 3) Regenerate short-term + long-term web dashboard pages
...
# Prefer project venv Python (symlink to ~/venvs/fengling-kanban is OK)
...
run_cmd "cd \"$ROOT_DIR\" && \"$PYTHON\" \"generate_daily_reports.py\" \
  --sales \"$SALES_FILE\" \
...
echo "== Step 2: regenerate short-term web dashboard =="
run_cmd "cd \"$ROOT_DIR\" && \"$PYTHON\" \"build_daily_web_dashboard.py\""
...
run_cmd "cd \"$ROOT_DIR\" && \"$PYTHON\" \"build_changqi_dashboard.py\" --source \"$CHANGQI_FILE\""
...
if [[ "$DRY_RUN" == "1" ]]; then
  ...
  cd "$ROOT_DIR"
  git add ".gitignore" ".nojekyll" ".github/workflows/deploy-pages.yml" \
    "build_daily_web_dashboard.py" "build_changqi_dashboard.py" "generate_daily_reports.py" \
    "update_and_publish.sh" "一键更新并发布.command" "config.env.example" "requirements.txt" \
    "风灵看板复用说明.md" "templates" "每日输出" \
    ...
    "changqi_history.csv" "changqi_detail_history.csv" "访问统计看板.html" "dashboard_history.csv" \
    "每日三表汇总看板_详情" "index.html"
